...
$.fn.productList=function(a){function b(a){var b=h+f,d=b+"id",e=b+"subid",k=b+"count";l.before('<table id="ptab'+f+'" class="ptab" val="'+f+'"><tr><td class="td"><input type="hidden" id="'+d+'" value="'+(a[0]||0)+'" /><input type="hidden" id="'+e+'" value="'+(a[1]||0)+'" /><table class="doptab"><tr><td class="lab">Ширина:<td><input type="text" class="size_x" /> м.<tr><td class="lab">Высота:<td><input type="text" class="size_y" /> м.<tr><td class="lab">Количество:<td><input type="text" id="'+k+'" value="'+
(a[2]||"")+'" class="count" maxlength="3" /> шт.</table><td class="td">'+(1<f?'<div class="img_del"></div>':"")+"</table>");var g=$("#ptab"+f);g.find(".img_del").click(function(){g.remove()});$("#"+d)._select({width:170,title0:"Не указано",spisok:CATEGORY_SPISOK,func:function(a){$("#"+e)._select("remove").val(0);0<a&&CATEGORY_SUB_SPISOK[a]&&c(a,e,k);$("#"+k).val(0<a?1:"").focus()}});c(a[0]||0,e,k);f++}function c(a,b,d){0!=a&&CATEGORY_SUB_SPISOK[a]&&$("#"+b)._select({width:160,title0:"Подкатегория не указана",
spisok:CATEGORY_SUB_SPISOK[a],func:function(){$("#"+d).focus()}})}var d=$(this),h=d.attr("id"),f=1,e;if("string"==typeof a&&"get"==a){a=d.find(".ptab");d=[];for(e=0;e<a.length;e++){var k=a.eq(e),g=h+k.attr("val"),k=$("#"+g+"id").val(),m=$("#"+g+"subid").val(),g=$("#"+g+"count").val();if(0!=k){if(!REGEXP_NUMERIC.test(g)||0==g)return"count_error";d.push(k+":"+m+":"+g)}}return 0==d.length?!1:d.join()}d.html('<div class="_product-list"><a class="add">Добавить поле</a></div>');var l=d.find(".add");l.click(b);
if("object"==typeof a)for(e=0;e<a.length;e++)b(a[e]);else b([]);return d};
$(document).ajaxSuccess(function(a,b,c){function d(){var a={op:"pin_enter",pin:$.trim($("#tpin").val())};a.pin?3>a.pin.length?(h("Длина пин-кода от 3 до 10 символов"),$("#tpin").focus()):(f.process(),$.post(AJAX_MAIN,a,function(a){a.success?f.close():a.max?location.reload():(f.abort(),h(a.text),$("#tpin").val("").focus())},"json")):(h("Не заполнено поле"),$("#tpin").focus())}function h(a){f.bottom.vkHint({msg:'<span class="red">'+a+"</span>",top:-47,left:62,indent:50,show:1,remove:1})}if(b.responseJSON.pin){var f=
