...
{op:"client_del",id:CLIENT.id};a.process();$.post(AJAX_MAIN,b,function(b){b.success?(a.close(),_msg("Клиент удален!"),location.href=URL+"&p=client"):a.abort()},"json")}})}));$("#stock").length&&($(".vkButton").click(function(){var a='<table id="stock-add-tab"><tr><td class="label">Категория:<td><input type="hidden" id="category_id" /><a href="'+URL+'&p=setup&d=stock" class="img_edit'+_tooltip("Настроить категорию комплектующих",-120)+'</a><tr><td class="label">Наименование:<td><input type="text" id="name" /></table>',
b=_dialog({top:70,width:450,head:"Внесение новой позиции склада",content:a,submit:function(){var a={op:"stock_add",category_id:$("#category_id").val(),name:$.trim($("#name").val())};a.name?(b.process(),$.post(AJAX_MAIN,a,function(a){b.abort();a.success&&(b.close(),stockSpisok())},"json")):b.bottom.vkHint({msg:'<SPAN class="red">Не указано наименование</SPAN>',left:110,top:-47,indent:50,show:1,remove:1})}});$("#category_id")._select({width:270,title0:"Категория не выбрана",spisok:STOCK_SPISOK});$("#name").focus()}),
$("#find")._search({width:250,focus:1,txt:"Быстрый поиск...",enter:1,func:stockSpisok}).inp(STOCK.find))});
